      ...
      <b-row class="bg-light">
        <div class="breadcrumb"  >
          <a v-for="s in crumbSections"
            ...
            v-bind:key="s.sectionId" :href="'#/admin/previewtest/'+testId"
            ...
            <b-col id="optionfontsize" style="margin-left:50px;">
              <b-form-radio-group v-model="questions[question_index-1].selected">
                <div v-for="(a, index) in  questions[question_index-1].answers" v-bind:key="index">
                    <b-form-radio :value="a.value"> <div v-html="a.text!=null?a.text.replace('__dexters_access_token_9836758498731097845__', $session.get('access_token')).replace('__dexters_resource_location_9836758498731097845__',resourceLocation):''"></div></b-form-radio>
                </div>
              </b-form-radio-group>
            </b-col>
          </b-row>
  ...
  <b-row>
      ...
      <b-row>
        ...
        <b-col cols="3" class="d-flex justify-content-center" style="padding:3px 0 3px 0;border:1px solid black;background: #e5f6fd;">
           <b-button class="btn-sm submit-btn" @click="showModal">Submit</b-button>
          </b-col>
      </b-row>
      </div>
  </b-row>
   <b-modal ref="myModalRef" hide-footer @hidden="onHidden">
     ...
     <b-row>
     <b-col class="d-flex justify-content-end">
    <b-btn @click="submit">Proceed to Submit</b-btn>
    </b-col>
     </b-row>
  </b-modal>
...
export default {
  ...
  async mounted() {
    ...
    if(this.answerId==null){
      ...
      await this.setLoaderMessage('Preparing your test');
  ...
  watch: {
    question_index: function(newValue, oldValue) {
      ...
      console.log('Calling MathJax',MathJax.Message);
      if (
  ...
  showModal () {
    ...
    getTestPack: function() {
      ...
      return new Promise((resolve, reject) => {
        axios
          .post("test/session/preview/start", data, config)
          .then(response => {
    ...
    resumeTest: function() {
      ...
      return new Promise((resolve, reject) => {
        axios
          .post("test/session/resume", data, config)
          .then(response => {
            this.testPack = response.data;
            resolve(response);
            // toast({
