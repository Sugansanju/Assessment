    ...
    <b-container style="margin-top: 20vh;" class="text-center">
        <b-row v-show="!isUserActivated && isUserActivated!=null">
            <b-col class="d-flex justify-content-center">
                ...
                <form @submit.prevent="fbUserRegister">
                    <div>
...
export default {
    props:['ftoken'],
    async mounted(){
        this.user.ftoken = this.ftoken;
        await this.checkFbUserStatus();
    },
    data: function(){
        return{
    ...
    methods:{
        checkFbUserStatus: function(){
            // console.log("Checking user status");
            return new Promise((resolve, reject) => {
            Account.faceBookStatus(this.ftoken)
                .then(response => {
        ...
        fbUserRegister: function(){
            if(this.user.password==this.user.confirmPassword){
            return new Promise((resolve, reject) => {
            Account.faceBookRegister(this.user)
                .then(response => {
                    if(response.data!=null && response.data!=""){
                        this.isUserActivated = true;
