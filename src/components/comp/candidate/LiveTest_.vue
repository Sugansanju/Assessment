<template>
<b-container fluid class="noselect" oncontextmenu="return false;" style="padding: 0px; overflow:hidden; scroll: none;">
  <b-row class="bg-dark">
    <b-col cols="9" class="p-1">
       <span class="d-flex justify-content-center bg-dark text-white">Test Title</span>
    </b-col>
    <b-col cols="3" >
      ...
      <span class="p-1 bg-dark text-white" v-b-modal.instructionsModal ><i class="fa fa-info-circle" aria-hidden="true"> Instructions</i></span>
    </b-col>
  </b-row>
    ...
    <b-col cols="9" style="background:white;">
      <b-row class="bg-light">
        <div class="breadcrumb"  >
          <a v-for="s in crumbSections" v-bind:key="s" :href="'#/liveTest/'+testId">{{s}}</a>
        </div>
      </b-row>
      ...
      <b-row>
        <b-container fluid style="overflow-y: scroll; height: 75vh; padding-bottom: 50px;">
          <b-row>
            <b-col>
        <b-container fluid>
          <b-row>
            <b-col style="padding:12px 16px 0px 25px;font-size:25px;" >
              <div v-html="questions[question_index-1].question.replace('__dexters_access_token_9836758498731097845__', $session.get('access_token')).replace('__dexters_resource_location_9836758498731097845__',resourceLocation)">
              </div>
          </b-col>
          </b-row>
        </b-container>
        <b-container fluid>
          <b-row>
            <b-col style="margin-left:50px;margin-top:25px;font-size:20px;">
              <b-form-radio-group v-model="questions[question_index-1].selected">
                <div v-for="(a, index) in  questions[question_index-1].answers" v-bind:key="index">
                    <b-form-radio :value="a.value"> <div v-html="a.text.replace('__dexters_access_token_9836758498731097845__', $session.get('access_token')).replace('__dexters_resource_location_9836758498731097845__',resourceLocation)"></div></b-form-radio>
                </div>
              </b-form-radio-group>
            </b-col>
          </b-row>
    ...
    <b-col cols="3"  style=" height: 95vh;border:1px solid black;">
      <b-row class="d-flex justify-content-around" style="height: 10vh;">
        <b-col style="">
          ...
          <span class="" style="font-weight:bold;font-size:30px;">
            {{currentUser.username!=null?currentUser.username:(currentUser.email!=null?currentUser.email:'User')}}
          </span>
        </b-col>
      </b-row>
      <b-row  style="height: 30vh;background: #e5f6fd;">
        <b-col class="diff_type_notation_area_inner" style="padding:12px;border:1px solid black;border-left:none;">
          <b-row >
            <b-col cols="5">
            ...
            <b-col cols="7" style="padding:0 12px 0px 1px; ">
              ...
              <b-col cols="12">
                <span class="review_marked">{{stats.answeredBookmarked}}</span>Answered & Marked for Review(Will not be considered for evaluation)
              </b-col>
            </b-row>
          </b-col>
        ...
        <b-row style="height: 50vh; overflow-y: scroll;background: #e5f6fd;margin-left:15px;">
            <answer-button-panel :buttons="buttons" @clicked="viewQuestion($event)"/>
        </b-row>
    </b-col>
  </b-row>
  <b-row>
      ...
      <b-row>
        ...
        <b-col>
          <b-button class="btn-sm mark-review-btn" @click="markReview">Mark for review & next</b-button>
           <b-button class="btn-sm clear-btn" @click="clearResponse">Clear Response</b-button>
        </b-col>
        <b-col class="d-flex justify-content-end">
           <b-button class="btn-sm save-btn" @click="saveAnswer">Save & Next</b-button>
        </b-col>
          </b-row>
          </b-col>
        <b-col cols="3" class="d-flex justify-content-center" style="padding:3px 0 3px 0;border:1px solid black;background: #e5f6fd;">
           <b-button class="btn-sm submit-btn" @click="submit">Submit</b-button>
          </b-col>
      </b-row>
      </div>
  </b-row>
...
<!-- <div class="noselect" oncontextmenu="return false;" onkeydown="return (event.keyCode != 116);" style="padding:0 0 0 0;">
<b-container fluid style="padding:0 0 0 0;overflow:hidden;">
      <b-row class="nav sticky-top " fixed="top">
        <b-col class="d-flex justify-content-center">
          <div style="color: #ffffff"><h3>{{testPack.sections[0].testName}}</h3></div>
        </b-col>
        <div class="d-flex jusitfy-content-end" style="margin-right:12px;">
              <b-col>
                <div class="questionpaper_icon"></div>
                <span class="nav_left" v-b-modal.questionsModal>Questions</span>
              </b-col>
              <b-col class="d-flex jusitfy-content-end">
                <span class="instruction_icon"></span>
                <a>
                  <span class="nav_left" v-b-modal.instructionsModal>Instructions</span>
                </a>
              </b-col>
        </div>
      </b-row>
      <b-container fluid style="position:fixed;">
      <b-row>
...
<div class="breadcrumb"  >
  <a v-for="s in crumbSections" v-bind:key="s" :href="'#/liveTest/'+testId">{{s}}</a>
          ...
          <b-col cols="2" style="border:1px solid black;border-top:none;border-bottom:none;background: #e5f6fd;" v-if="$session.exists('current_user')">
             <div class="d-flex jusitfy-content-end">
            <b-col class="user_pic" style="padding:0 0 0 0;">
            <img width="94" height="95" src="/static/images/user.png">
            </b-col>
          <b-col style="margin-top:20px;"  v-if="currentUser!=null">
            <span class="" style="font-weight:bold;font-size:30px;">
              {{currentUser.username!=null?currentUser.username:(currentUser.email!=null?currentUser.email:'User')}}
            </span>
          </b-col>
       ...
       <b-row>
        <b-col cols="10" style="padding:0 0 0 0 ;margin-top:-35px;">
        <b-col style="border-top:1px solid black;border-bottom:1px solid black;font-weight:bold;">
          <span>Question No : {{question_index}}</span>

          <timer v-if="timeDuration!=0" :time="timeDuration" class="pull-right" @timeAlert="timeAlert"  @timeExpired="timeExpired"></timer>

        </b-col>
        <div id="right-pane" v-if="questions[question_index-1].question!=null">
        <b-col style="padding:12px 16px 0px;font-size:25px;" >
          <div v-html="questions[question_index-1].question.replace('__dexters_access_token_9836758498731097845__', $session.get('access_token')).replace('__dexters_resource_location_9836758498731097845__',resourceLocation)">
          </div>
        </b-col>
        <b-col style="margin-left:50px;margin-top:25px;font-size:20px;">
      <b-form-radio-group v-model="questions[question_index-1].selected">
        ...
        <b-col cols="2" style="padding:0 0 0 0;background: #e5f6fd;">
        <b-col class="diff_type_notation_area_inner" style="padding:10px 10px 10px 10px;font-size:13px;border:1px solid black;">
          <b-row style="">
            <b-col>
              <span class="answered">{{stats.answered}}</span>Answered
            </b-col>
            <b-col style="padding:0 12px 12px 1px; ">
          <span class="not_answered">{{stats.notAnswered}}</span>Not Answered
          </b-col>
          </b-row>
          <b-row>
        ...
        <b-col style="font-size:14px;font-weight:bold;background: #e5f6fd;border:1px solid black;border-bottom:none;">
          <br><span>Choose a Question</span>
            <b-container style="height:600px;overflow-y:scroll;margin-top:12px;">
            <b-row>
            <answer-button-panel :buttons="buttons" @clicked="viewQuestion($event)"/>
            </b-row>
      ...
      <div class="footer">
      <b-row>
        <b-col cols="10"  style="border:1px solid black;border-right:none;padding:3px 3px 3px 15px;">
          <b-row class="d-flex justify-content-start">
        <b-col>
          <b-button class="btn-sm mark-review-btn" @click="markReview">Mark for review & next</b-button>
           <b-button class="btn-sm clear-btn" @click="clearResponse">Clear Response</b-button>
        ...
        <b-col cols="2" class="d-flex justify-content-center" style="padding:3px 0 3px 0;border:1px solid black;background: #e5f6fd;">
           <b-button class="btn-sm submit-btn" @click="submit">Submit</b-button>
          </b-col>
      </b-row>
      </div>
  ...
  methods: {
    ...
    submitTest: function(){
      ...
      return new Promise((resolve, reject) => {
        axios.post("test/session/submit", data, config)
          .then((response) => {
            resolve(response);
            toast({
              type: "success",
    ...
    processStats: function(){
      ...
      for(let i=0;i<this.buttons.length;i++){
        ...
        if(b.answered)
          ...
          this.stats.notAnswered++;

        if(b.visited)
          this.stats.visited++;
        else
          this.stats.notVisited++;
        if(b.answered && b.bookmarked)
          this.stats.answeredBookmarked++;
        if(b.bookmarked)
          this.stats.bookmarked++;
      }
    ...
    saveUpdateAnswer: function(index){
      ...
      let data={
        ...
        answerData: {
          ...
          answerNumber: this.questions[index].selected!=null ? this.questions[index].selected.answerNumber : null,
          answer: this.questions[index].selected!=null ? this.questions[index].selected.answer : null,
      ...
      return new Promise((resolve, reject) => {
        axios.post("test/session/answer/save", data, config)
          .then((response) => {
            this.processStats();
            resolve(response);
            // toast({
    ...
    timeExpired: function(){
      swal({
        ...
        if (result.value) {
          this.submitTest();
          this.$router.push("/candidate/result/"+this.testPack.answerId);
        }
      })
    },
    getTestPack: function(){
      ...
      return new Promise((resolve, reject) => {
        axios.post("test/session/start", data, config)
          .then((response) => {
    ...
    prepareTest: function(){
      this.isLoading=true;
      console.log('Preparing test pack');
      this.title.splice(0);
      this.title.push({
      ...
      this.timeDuration=0;
      for(let i=0, count=1;i< this.testPack.sections.length;i++){
        ...
        this.section.push({
          ...
          for(let j=0;j<s.questionData.length;j++){
            ...
            this.buttons.push( { index: count++, visited: false, bookmarked: false, answered: false });
          }
      }
